                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <a href="{{route('admin.groups.create')}}" class="btn btn-success float-right" title="{{__('Thêm mới nhân viên')}}">{{__('Thêm mới nhân viên')}}</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered dt-responsive" style="table-layout: fixed;width: 100%;">
                                    ...
                                    @foreach($contacts as $key => $contact)
                                        <tr>
                                            ...
                                            <td>{{$contact->name}}</td>
                                            <td>{{$contact->phone}}</td>
                                            <td>{{$contact->type === 0 ? 'Kinh doanh' : 'Kỹ thuật'}}</td>
                                            <td>{!! \App\Helpers\Helper::active($contact->active) !!}</td>
                                            <td>
                                                <a class="btn btn-info btn-sm btn-edit" href="{{route('admin.groups.edit', $contact->id)}}"><i class="fas fa-pencil-alt"></i></a>
                                                <button class="btn btn-danger btn-sm btn-delete" data-url="groups/destroy" onclick="removeFunction('{{$contact->id}}')"><i class="fas fa-trash"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
